...
consumer_key = "YiDQahzLJqAaA1AHjmd3VPsG0FyvmdrSIxUArWeSWpRaKbvN"
consumer_secret = "joM43bF1lk9OVcBbOaCRALs7bJz6nXaAWwEYl94AzLr4GLymA6392vN1Kw5coGUc"
...
def sendSTK(phone_number, amount, orderId, transaction_id=None, shortcode=None, account_number=None):
    code = shortcode or SHORT_CODE
    print("this is the pshort code")
    print(SHORT_CODE)
    print("this is the till number")
    print(TILL_NUMBER)
    print(api_settings.TILL_NUMBER)
    party_b = int(TILL_NUMBER)
    ...
    if account_number:
        transaction_type = "CustomerBuyGoodsOnline"
    elif transaction_type == "CustomerBuyGoodsOnline" and account_number == None:
    ...
    print("this  is what has been sent back through call back")
    ...
    if json_response.get('ResponseCode'):
        if json_response["ResponseCode"] == "0":
            ...
            if transaction_id:
                transaction = PaymentTransaction.objects.filter(id=transaction_id).get()
                transaction.checkout_request_id = checkout_id
                transaction.save()
                return transaction.id
            else:
                transaction = PaymentTransaction.objects.create(phone_number=phone_number,checkout_request_id=checkout_id,
...
def check_payment_status(checkout_request_id, shortcode=None):
    ...
    if 'ResponseCode' in json_response and json_response["ResponseCode"] == "0":
        ...
        if transaction and json_response['ResultCode'] == "0":
            print("here")
