...
from rlextra.utils.utils import find_exe, remove_files
verbose=0
...
def gsExeFind(gsExe=None):
    if not gsExe:
        ...
        if sys.platform=='win32':
            ...
            exeName = 'gs'
        gsExe = find_exe(exeName,extra_path=known_places)
...
def _run(ARGS,input=None):
    if verbose>2:
        cmd = ' '.join((_ if ' ' not in _ else f'{_!r}' for _ in ARGS))
        print(f'_run({cmd})',file=sys.stderr)
...
def epsBBox(fn,bbpat=re.compile(rb'^\s*%%boundingbox:\s*(.*)\s*$',re.I|re.M)):
    '''
