...
class Restart(SubConfig):
    def __init__(self,**kw):
        kw.setdefault('attrInfo',None)
        kw.setdefault('srcInfo',None)
        kw.setdefault('logInfo',None)
        kw.setdefault('projectInfo',None)
        super().__init__(**kw)

    def __bool__(self):
        return bool(self.projectInfo)
...
_fakeIO_stdout = _fakeIO_stderr = None
_initLogText=[]
...
class Project:
    ...
    def _pathNWD(self):
        if not self.path:
            nwd = os.path.dirname(self.path)
            autoCd(nwd)
            addSearchPaths(getWD(nwd))
            return nwd

    def open(self,path=None,ckm=True):
        if ckm and not self._checkModified('Open'): return
        if not path:
...
class GUIEditor:
    def __init__(self, top):
        ...
        menu.addmenuitem('Actions', 'command', 'Restart',
    ...
    def saveConfig(self):
        ...
        x = (ws/2) - (w/2)
        y = (hs/2) - (h/2)
        config.root.geometry = f'{int(w)}x{int(h)}+{int(x)}+{int(y)}'
        self.pack_forget()
        config.save()
    ...
    def restart(self):
        ...
        if project.className:
            config.restart = Restart(
                attrInfo = self.saveAttrInfo(),
                projectInfo = pickle.dumps(self.project),
                srcInfo = self.saveTextWindow('src'),
                logInfo = self.saveTextWindow('log'),
                )
        else:
            config.restart = Restart()
        ...
        os.execl(python, python,'-mrlextra.graphics.guiedit.guiedit',f'--config={config._path}')
        root.destroy()
    ...
    def saveTextWindow(self,name):
        if getattr(getattr(config,name,None),'on',False):
            win = getattr(self,name)
            return (win.get(),
                tuple(((_,)+tuple(map(str,win._textbox.tag_ranges(_))) for _ in win._textbox.tag_names())),
                (win.xview()[0],win.yview()[0]),
                )

    def restoreTextWindow(self,name,saves):
        if saves is None: return
        if getattr(getattr(config,name,None),'on',False):
            win = getattr(self,name)
            win.settext(saves[0])
            for t in saves[1]:
                if len(t)>1:
                    win._textbox.tag_add(t[0],*t[1:])
            win.xview_moveto(saves[2][0])
            win.yview_moveto(saves[2][1])

    def saveAttrInfo(self):
        if config.attrList.on:
            attrList = self.attrList
            return (self._attrStack, self.attrRootName, int(attrList.curselection()[0]))

    def restoreAttrInfo(self,saves):
        if config.attrList.on and saves:
            for name, active in saves[0]:
                self.attrRootName = name
                self.newAttrList(active)
            self.attrRootName = saves[1]
            self.newAttrList(saves[2])
            self._attrStack = saves[0]
    ...
    def findObjectsFirst(self):
        ...
        self.findObjects()
        if config.restart:
            self.project = pickle.loads(config.restart.projectInfo)
            self.project.notify = self.redraw
            self.project._pathNWD()
            self.setMain('display')
            self.project.notify(self,'redraw')
            self.restoreTextWindow('src',config.restart.srcInfo)
            self.restoreTextWindow('log',config.restart.logInfo)
            self.restoreAttrInfo(config.restart.attrInfo)
    ...
    def editPythonPath(self):
        ...
        if r != 'OK': return
        L = [os.path.expanduser(_) if _.startswith('~') else _ for _ in d.getList()]
...
def _getConfig(fn=None):
    ...
    if not config:
        ...
        writeLog('info',f'Reading config from {fn!r}')
        config = Config(fn,
                    package = SubConfig(
...
def mainApp():
    global _fakeIO_stdout, _fakeIO_stderr
    _fakeIO_stdout = sys.stdout = _fakeIO('info')
    _fakeIO_stderr = sys.stderr = _fakeIO('debug')
    ...
    try:
        ...
        if fn:
            fn = fn[-1][9:] or None
            sys.argv[1:] = [x for x in sys.argv[1:] if not x.startswith('--config=')]
        else:
            fn = None
    except:
        fn = None
