...
from rlextra.utils.cgisupport import cgi
...
class record:
    ...
    def readRecord(self, sock):
        ...
        if paddingLength != 0:
            #padding = sock.recv(paddingLength) #this is currently unused   #this is currently unused
            pass
...
def readPair(s, pos):
    ...
    if nameLen & 128:
        b=list(map(ord, s[pos:pos+3])) ; pos=pos+3
...
def HandleManTypes(r, conn):
    if r.recType == FCGI_GET_VALUES:
        ...
        for i in list(r.values.keys()):
            if i in vars: v[i]=vars[i]
        r.values=vars
        r.writeRecord(conn)
...
class FCGI:
    def __init__(self):
        ...
        if 'FCGI_WEB_SERVER_ADDRS' in os.environ:
            ...
            good_addrs=list(map(str.strip,good_addrs))        # Remove whitespace
        else:
            good_addrs=None
    ...
    def getFieldStorage(self,keep_blank_values=0):
        method = 'GET'
        if 'REQUEST_METHOD' in self.env:
            method = self.env['REQUEST_METHOD'].upper()
        if method == 'GET':
            return cgi.FieldStorage(environ=self.env, keep_blank_values=keep_blank_values)
        else:
            return cgi.FieldStorage(fp=self.inp, environ=self.env, keep_blank_values=keep_blank_values)
...
class CheckFileTimeStamps:
    def __init__(self,*files):
        self.ts = dict([(f,None) for  f in files])
        self(True)
...
class SaveState():
    def __init__(self):
        ...
        self.__state = dict(
            ...
            binn = list(builtins.__dict__.keys()),
            )
    ...
    def restore(self):
        ...
        for k,v in self.__state.items():
            if k=='binn':
                for x in list(builtins.__dict__.keys()):
                    if x not in v:
                        delattr(__builtins__,x)
            else:
                obj = getattr(sys,k)
                if isinstance(v,dict):
                    for x in [x for x in obj if x not in v]:
                        del obj[x]
                else:
                    obj[:] = v
