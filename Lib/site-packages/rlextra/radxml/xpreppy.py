...
from reportlab.lib.rl_safe_eval import rl_less_safe_eval
...
def preProcess(tree, nameSpace, caller=None):
    ...
    for key, value in list(newAttrs.items()):
        if isinstance(value,str) and value[0:1] == '$':
            newValue = rl_less_safe_eval(value[1:], nameSpace)
            newAttrs[key] = newValue
    attrs = newAttrs

    if tagName in TAG_LOOP:
        innerTxt = attrs[TAG_LOOP_INNER]
        outer = rl_less_safe_eval(attrs[TAG_LOOP_OUTER], nameSpace)
        dataSet = []
        for row in outer:
        ...
        try:
            value = rl_less_safe_eval(valueStr, nameSpace)
        ...
        exprValue = rl_less_safe_eval(exprText, nameSpace)
        ...
        yesOrNo = rl_less_safe_eval(condText, nameSpace)
        if yesOrNo:
        ...
        if exprText:
            expr = rl_less_safe_eval(exprText, nameSpace)
            ...
            if isinstance(child,tuple):
                ...
                if childTagName in TAG_CASE:
                    ...
                    if exprText:
                        ...
                        try:
                            value = rl_less_safe_eval(condition, nameSpace)
                        except NameError:
                        ...
                        yes = rl_less_safe_eval(condition, nameSpace)
                        if yes:
                            selected = processChildren(grandChildren, nameSpace)
                            break
        ...
        if dataStr:
            dataObj = rl_less_safe_eval(dataStr, nameSpace)
        else:
            dataObj = nameSpace
...
def run(sample):
    ...
    tree = xml2TT(sample,returnUTF8=0)
